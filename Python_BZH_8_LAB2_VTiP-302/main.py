...
class Main(QDialog):
    def __init__(self):
        ...
        loadUi('front.ui', self)  # Загрузка формы из файла
        ...
        self.label_img.setPixmap(QPixmap('task8.png'))
    ...
    def solve(self):
        try:
            ...
            if x <= 9:
                answer = math.pow(a+b, 2) / math.pow(x, 2)
            else:
                answer = x * (math.pow(a, 2) + math.pow(b, 2))
